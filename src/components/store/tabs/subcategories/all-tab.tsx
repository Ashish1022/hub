...
import { SubcategoryColumn } from '@/app/store/(dashboard)/[storeId]/(routes)/subcategories/_components/columns'
...
const AllTab = ({ data }: { data: SubcategoryColumn[] }) => {
                        ...
                        <tbody>
                            {data.map((subcategory, i) => (
                                <tr key={i} className="border-b border-[#1E293B] last:border-0">
                                    ...
                                    <td className="py-3 px-4 text-sm text-[#A4B8D3]">{subcategory.products}</td>
                                    <td className="py-3 px-4">
                                        <Badge
                                            className={
                                    ...
                                    <td className="py-3 px-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <Button variant="ghost" size="icon" className="h-8 w-8 cursor-pointer" onClick={() => router.push(`/store/${storeId}/products/${subcategory.id}`)}>
